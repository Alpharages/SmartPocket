...
| SP-014 | High | Functional | Data layer | (all) | **Database failures are silently rendered as "no data".** Nearly every read in `server/db.ts` ends `catch { return [] }` / `catch { return {...zeros} }` (e.g. `getUserCategories:166`, `getUserTransactions:711`, `getAccountBalances:1350`, `getMonthlyStats:1394`). A DB outage, a bad credential or a query error is indistinguishable from a genuinely empty account. | Stop the database and open the app. | An error state: "Couldn't load your data — retry". | Every screen shows its friendly empty state ("No transactions yet", "Add your first income or expense"). A user with 3 years of history is told they have none — and may then add duplicates. | Let read errors propagate as tRPC errors, surface them via the existing `ToastProvider` plus a retry affordance, and reserve empty states for genuinely empty result sets. | ✅ **Fixed & verified** |
...
All 72 issues were worked. **62 are fixed and verified**; **10 are deliberately
not fixed** and are listed below with the reason. Verification method is stated
per item — nothing is marked fixed on the strength of "the code looks right".

### B.1 Verification baseline

| Gate | Before | After |
| --- | --- | --- |
| Vitest | 1307 passed, **1 failed**, 1 skipped | **1324 passed, 0 failed**, 1 skipped |
| `tsc --noEmit` | clean | clean |
| ESLint | **9 errors**, 58 warnings | **0 errors**, 77 warnings |
| Cross-tenant attack script | **5 of 5 attacks succeeded** | **0 of 5** |

Two new regression suites were added that fail against the old code:
`tests/authorization.cross-user.test.ts` (12 cases) and
`tests/lib/expense-context.monthly-stats-period.test.tsx` (3 cases).

### B.2 How each class was verified

| Class | Method |
| --- | --- |
| Authorization (SP-001/002/023) | The original attack script re-run: 5/5 → 0/5, with the correctly-scoped `accounts` router as an unchanged control. Plus 12 new router-level tests. |
| API validation (SP-012/031/033/041) | Live HTTP calls against the running API — zero amount, non-numeric PAN, Luhn-invalid PAN, expired expiry and duplicate category name all rejected with specific messages. |
| CORS / cron (SP-005/011) | `curl`: disallowed origin → **403** with no `Allow-Origin`; allowed origin → 204 with it. Unauthenticated cron POST → **401**. |
| Layout and contrast (SP-057/058/059/060/062/063/065/066) | Computed styles and colours read from the live DOM in Chromium at 430×932 and 1440×900, light and dark. |
| Interaction (SP-003/007/009/016/027/037/045) | Driven in the browser: rows deleted, sheets opened, months navigated, searches run. |
| Everything else | Vitest + `tsc` + ESLint, with tests updated where they encoded the old behaviour. |

### B.3 Tests that encoded the bugs

Seven existing tests passed *because* of a defect and had to be rewritten. They
are worth calling out, because they are why several of these issues survived:

| Test | What it asserted | Why that was wrong |
| --- | --- | --- |
| `transactions.test.tsx` delete cases | That `Alert.alert`'s destructive callback, invoked by hand, calls `deleteTransaction` | The callback never fires on web, and the affordance was off-screen anyway — the test could pass while the feature was unusable |
| `server/accounts.test.ts`, `monthly-trend`, `summary-anomalies` | That a DB failure **resolves to `[]`** | Encoded SP-014 directly: an outage was *specified* to look like empty data |
| `expense-context.transaction-stats` (×3) | That `monthlyStats.refetch()` was called | Asserted the mechanism of the SP-003 bug rather than the outcome |
| `summary.test.tsx` next-month | That you can page past the current month | Encoded the unbounded navigation of SP-045 |
| `app.tabs-layout.test.tsx` | Read `options.tabBarAccessibilityLabel` | Asserted config, not rendered output — so SP-061 (the label never reaching the DOM) was invisible to it |

### B.4 Not fixed — and why

These are **not** oversights. Each is either a product decision that is not
mine to make, or architectural work whose scope exceeds a defect-fix pass.

| ID | Sev | Item | Why it was not done |
| --- | --- | --- | --- |
| SP-013 | High | Session token in URL / `localStorage` on web | Requires changing the OAuth redirect contract so the token is only ever set as an HTTP-only cookie server-side. That is a coordinated client+server auth change with a real migration risk for existing sessions, and it should ship on its own with a rollback plan. The CSRF nonce (SP-022) and the CORS allowlist (SP-005) close the most exploitable part of the surface in the meantime. |
| SP-015 | High | Unbounded `transactions.list` | Needs pagination in the API plus windowing in four consuming screens, and Insights/Budgets currently derive from the full in-memory array. Real work, and it changes behaviour users can see. |
| SP-019 | High | Multi-currency model is unsound | **A product decision.** Either add `currency` to transactions with conversion, or constrain accounts to the global currency. Option two is a data-model change and option one needs an FX source. Someone has to choose. |
| SP-030 | Medium | No category icon picker | Feature work, not a defect fix. The field round-trips already. |
| SP-035 | Medium | Free-text dates instead of native pickers | Date fields are now *consistent* across the app (Add Transaction and Record Repayment gained one), but a shared native date-picker component is a design task spanning seven fields. |
| SP-038 | Medium | Loan interest never applied | **A product decision**: implement amortisation, or drop the Rate field. Shipping a finance feature that displays a rate it does not honour is the current state, and I would not remove a field without the owner's call. |
| SP-043 | Medium | Balances recomputed from all transactions in JS | Same class as SP-015 — move aggregation into SQL. |
| SP-052 | Low | App name / bundle identifier mismatch | The bundle ID is **immutable after first store submission**. Changing `com.app.expensetrackerapp` is an owner decision, not a QA fix. |
| SP-055 | Low | Category drill-down is desktop-only | Feature work: needs a filtered-Activity route for phones. |
| — | — | AI features, localisation, full backup/restore | PRD-declared roadmap items (§12.3), never built. Out of scope for a defect pass. |

### B.5 One finding was mis-attributed, and the fix is bigger than reported

**SP-007** was reported as "delete uses `Alert.alert`, whose buttons are inert
on web". That is true, and it is fixed. But running the fix revealed the
*primary* cause was different and worse: edit and delete existed **only** inside
`Swipeable`'s `renderRightActions`, which on web renders off-screen — measured
at **x = −9638** — with no swipe gesture to bring it in. The affordance was not
merely inert, it was **physically unreachable**, and my original click during
the audit was landing on negative coordinates.

Both are now fixed: `TransactionRow` renders persistent trailing actions where
swipe is not an input method, and all three call sites go through a shared
platform-safe confirm helper. Verified in a browser — the list went from 5 rows
to 4 on confirm.

This is worth recording because it is the one place the static audit reached

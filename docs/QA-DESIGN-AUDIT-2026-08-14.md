...
>
> **Every finding in this audit (SP-087 … SP-094) was fixed and re-verified on 2026-08-14 in
> commit `fe105f7`.** Per-issue evidence is in `docs/qa-evidence/qa-2026-08-14-design-bugs.csv`.
>
> The three documentation findings were resolved **by correcting the documents, not the code** —
> in each case the implementation was right. The `success`/`warning`/`accent` values are
> contrast-hardened deliberately, so both specs now carry a note recording why they must not be
> "restored" to the lighter published values. A token-diff check confirms every colour, type and
> radius token in `theme.config.js` now appears in the design docs.
>
> Highlights: touch targets went **86/90 → 90/90**; the balance hero renders from a named
> `Typography.hero` token instead of a `fontSize: 42` literal; a 1366×768 non-retina laptop now
> keeps the hero gradient (3 new unit tests pin the desktop/touch split); and the app finally
> ships under one name.
>
> **The findings below are left as originally written**, as the record of what was measured on
> 2026-08-14. §8 (What Was NOT Audited) is unchanged and still applies — no Figma comparison,
> no Dynamic Type verification, no native rendering.

---

## 1. Executive Summary

**The design system itself is in good shape. The design _documentation_ is not.**
...
**Of the five "Known Consistency Fixes" listed at the bottom of `design.md`, four were already
done at audit time; the fifth — unifying the app name to "SmartPocket" — was outstanding as
SP-093 and has since been closed. All five are now complete.**

**Design conformance verdict (as audited 2026-08-14): 🟡 Good system, stale documentation.**
Nothing here blocked a release on visual grounds; the touch-target failures (SP-091) were the
only findings with a direct accessibility impact.

**Current: ✅ closed.** Documentation re-synced to the code, tokens and touch targets fixed — see
the resolution banner above.

### Findings at a glance

| ID     | Severity | Area               | Finding                                                                                                          | Status    |
| ------ | -------- | ------------------ | ---------------------------------------------------------------------------------------------------------------- | --------- |
| SP-087 | Medium   | Typography         | Type scale in both specs disagrees with the shipped tokens (4 of 8)                                              | **Fixed** |
| SP-088 | Medium   | Colour             | `success`, `warning`, `accent` token values drifted from both specs                                              | **Fixed** |
| SP-089 | Medium   | Colour / Docs      | Hero gradient is 3-stop with a cyan stop; specs document a 2-stop indigo→violet. Three-theme system undocumented | **Fixed** |
| SP-090 | Medium   | Typography         | `fontSize: 42` hardcoded in `BalanceHero`, bypassing the type scale                                              | **Fixed** |
| SP-091 | Medium   | Accessibility      | 4 controls below the spec's 44×44 minimum touch target                                                           | **Fixed** |
| SP-092 | Low      | Visual consistency | Device-tier heuristic disables the hero gradient on 1366×768 non-retina laptops                                  | **Fixed** |
| SP-093 | Low      | Branding           | App name still "Expense Tracker"; `design.md` lists unifying it to "SmartPocket" as a required fix               | **Fixed** |
| SP-094 | Low      | Spacing / Radius   | Off-token radii (24px, 28px) and off-grid spacing (14, 30, 34px)                                                 | **Fixed** |
...
| **Hero gradient renders** (`primary → secondary`, dashboard only)                                    | ✅ Renders correctly at every realistic device pixel ratio — verified at dpr 2, dpr 3, and 1440×900/1920×1080 desktops. Gradient appears only on the dashboard. _(Stop values diverge — see SP-089.)_                             |
...
| --------- | -------------- | ---------------------------- | ----------------------------------- | ----- |
| `display` | 32 / 38 · 700  | **36 / 40** · 700            | _(bypassed — see SP-090)_           | ❌    |
...
Both specs state: _"Hero gradient: `primary → secondary` (indigo → violet), used only on the
dashboard balance card."_ That describes **two stops**, `#4F46E5 → #7C3AED`.
...
Both specs require: _"Minimum touch target: 44×44 pt"_ / _"Touch targets ≥ 44×44 pt"_.
...
broadly similar capability — keeps it. The heuristic conflates _few pixels_ with _weak GPU_,
which holds for phones but inverts on desktops: a low-resolution desktop has **fewer** pixels to
push, so it should find the gradient _easier_, not harder.
...
**Note on method:** this finding is also why an earlier observation was _not_ filed — testing at
...
`design.md`'s own "Known Consistency Fixes" list requires: _"Unify app title to **SmartPocket**
(currently shows 'Expense Tracker')."_ It is the only one of the five still outstanding.
...
- The same screen simultaneously says _"Lets **SmartPocket** suggest categories…"_
...
| 5   | Unify the app title to "SmartPocket"                                 | ✅ **Done** — closed as SP-093 in `fe105f7`; `appName` is now `SmartPocket` (the EAS slug is deliberately unchanged) |

**5 of 5 complete** (4 at audit time, the last closed in `fe105f7`). This list has been worked through diligently.
...
3. **Record _why_ the contrast-hardened colours differ**, so they are not "corrected" back to the
   documented values by a future contributor.

**Implementation** 4. **Fix the four sub-44×44 touch targets** (SP-091) — the only design finding with a direct
accessibility impact. Use `hitSlop`; the Activity pills are the reference. 5. **Move the balance hero onto a type token** (SP-090). 6. **Scope the device-tier heuristic to touch platforms** (SP-092) so low-DPI laptops keep the
gradient. 7. **Set the app name to "SmartPocket"** (SP-093), coordinating the slug change with OAuth
redirect URIs. 8. **Sweep the off-token radii and spacing literals** (SP-094) and consider a lint rule to hold
the line.

**Process** 9. Add a **token-conformance test** — assert that rendered heading sizes match `Typography`, and
...
_Design QA performed with the Elite QA Engineer skill, Mode C — Design QA. Every conformance
...
`sp-092-gradient-lost-1366x768-laptop.png`, `qa-2026-08-14-dashboard-dark-phone.png`._
